...
(function (global) {
  ...
  const WORK_TYPES = {
    ...
    tank_inspection: {
      ...
      // Backed by ctk_tanks.json: nearly everything about a tank is fixed
      // between visits, so the technician picks the site and confirms the
      // handful of things that actually change.
      site_source: 'reference',
      reference: 'ctk_tanks',
      client_locked: null,
      generator: 'generate_tank_report.py',
      legacy_export: 'tank',
      status: 'live',
    },
    temperature_monitoring: {
      ...
      // Backed by ctk_outlets.json. The outlet list at a site is stable, and
      // which readings apply is a property of the outlet - a sink takes hot
      // and cold, a thermostatic mixer takes a TMV reading only.
      site_source: 'reference',
      reference: 'ctk_outlets',
      client_locked: null,
      generator: 'generate_temp_report.py',
      legacy_export: 'temp',
      status: 'live',
    },
    legionella_ra: {
  ...
  function emptyDataFor(type) {
    ...
    if (type === 'temperature_monitoring') {
      return {
        site_code: null,
        doc_code: null,
        comments: '',
        outlets: [],      // { name, group, readings[], hot, cold, tmv, hot_return, notes }
      };
    }
    if (type === 'tank_inspection') {
      return {
        site_ref: null,          // key into ctk_tanks.json
        report_id: null,         // stem + MMYY, worked out when the visit starts
        overview: '',            // optional; usually written back at the office
        tanks: [],               // one entry per tank inspected
      };
    }
  ...
  /**
   * Report ID: the site's stem plus the month and year of the inspection.
   * e.g. DBD10TI + '0426'. Zero-padded — one of the existing reports has a
   * month without its leading zero and it reads as an error.
   */
  function tankReportId(stem, isoDate) {
    if (!stem) return null;
    const m = /^(\d{4})-(\d{2})-\d{2}$/.exec(String(isoDate || ''));
    if (!m) return stem;
    return stem + m[2] + m[1].slice(2);
  }

  /**
   * Out-of-range readings, shown in red on the report.
   * Hot isn't checked - low hot readings during peak demand are expected on
   * a busy site and get explained in the comments rather than flagged.
   */
  const TEMP_LIMITS = {
    cold: { max: 20, label: 'above 20\u00b0C' },
    // 45 is the scalding threshold at a mixed outlet, not 55 - a TMV
    // delivering 50 is a burn risk even though it isn't a legionella one.
    tmv:  { max: 45, label: 'above 45\u00b0C' },
  };
  ...
  function splitForGenerators(bundle) {
    ...
    for (const v of bundle.visits || []) {
      for (const wi of v.work_items || []) {
        ...
        else if (wi.type === 'tank_inspection') tankVisits.push({ visit: v, item: wi });
        else if (wi.type === 'temperature_monitoring') tempVisits.push({ visit: v, item: wi });
      }
    }
    ...
    if (tankVisits.length) out.tank = tankVisits.map((p) => toLegacyTank(p, bundle));
    if (tempVisits.length) out.temp = tempVisits.map((p) => toLegacyTemp(p, bundle));
    return out;
  }
  ...
  /** Bundle for the temperature monitoring generator. */
  function toLegacyTemp(pair, bundle) {
    const v = pair.visit;
    const d = pair.item.data || {};
    return {
      exportType: 'ctk_temperature_monitoring',
      schema_version: 1,
      exportedAt: bundle.exported_at,
      visit: {
        visit_id:  v.visit_id,
        ...
        client:    v.client || '',
        date:      v.date,
        engineer:  v.technician,
        comments:  (d.comments || '').trim(),
      },
      outlets: (d.outlets || []).map((o) => ({
        name:       o.name,
        group:      o.group || 0,
        readings:   o.readings || [],
        hot:        o.hot,
  ...
  /**
   * Bundle for the tank report generator. Carries only what was recorded on
   * site — the generator merges it with ctk_tanks.json for everything fixed,
   * so the bundle stays small and there's one place the static data lives.
   */
  function toLegacyTank(pair, bundle) {
    const v = pair.visit;
    const d = pair.item.data || {};
    return {
      exportType: 'ctk_tank_inspection',
      schema_version: 1,
      exportedAt: bundle.exported_at,
      inspection: {
        visit_id:    v.visit_id,
        site_ref:    d.site_ref || v.site_ref || v.site,
        site:        v.site,
        client:      v.client || '',
        address:     v.address || '',
        date:        v.date,
      ...
      tanks: (d.tanks || []).map((t) => ({
        tank_ref:     t.tank_ref || null,
        // Present only on a first survey: the tank's permanent details, which
        // the generator writes into the register so later visits are short.
        fixed:        t.fixed || null,
        ambient_c:    t.ambient_c,
        makeup_c:     t.makeup_c,
        storage_c:    t.storage_c,
        clarity:      t.clarity,
        stagnancy:    t.stagnancy,
        biofilm:      t.biofilm,
        deposits:     t.deposits,
  ...
  function toLegacyInstall(pair, bundle) {
    ...
    // Devices finished on an earlier day are carried forward so the
    // technician can see them, but they belong to THAT day's bundle — with
    // its photos. Including them here would produce a bundle that looks
    // complete on its own while quietly missing the evidence.
    const devices = [];
    let carriedForward = 0;
    for (const section of d.sections || []) {
      for (const dev of section.devices || []) {
        if (dev.carried_from) { carriedForward++; continue; }
        devices.push({
          // The code recorded on site wins over the scheduled one
          code: dev.deviceCode || dev.code || '',
          name: dev.name || '',
          section: section.name || '',
          status: dev.status || null,
          notes: dev.notes || '',
          channels: dev.channels || [],
          photos: (dev.photos || []).map((p) => p.data || p),
          ts: dev.ts || null,
        });
      }
    }
    ...
    return {
      job: {
        ...
        ref: d.ref || '',
        gateways: (d.gateways || [])
          .filter((gw) => !gw.carried_from)
          .map((gw) => ({
            id: gw.id,
            code: gw.code || '',
            location: gw.location || '',
            status: gw.status || null,
            photos: (gw.photos || []).map((p) => p.data || p),
          })),
        // Informational: says this is one day of a longer job, so whoever
        // picks the file up knows to look for its siblings.
        carriedForward: carriedForward,
        multiDay: carriedForward > 0,
