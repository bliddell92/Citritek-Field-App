...
(function (global) {
  ...
  const BUILD = '0.9.4';   // must match ctk_schema.js and index.html
  ...
  const REFERENCE_SOURCES = {
    ...
    // Tank register. Optional: a deployment doing only Bristol Water work
    // shouldn't fail to start because this isn't there.
    ctk_tanks:    { file: 'ctk_tanks.json',       required: false, fallback: { schema_version: 1, sites: [] } },
    ctk_outlets:  { file: 'ctk_outlets.json',     required: false, fallback: { schema_version: 1, sites: [] } },
  };
  ...
  const loadTanks = () => loadRef('ctk_tanks');
  const loadOutlets = () => loadRef('ctk_outlets');
  ...
  const CTKStorage = {
    ...
    loadSites, loadRemedials, loadDevices, loadProcessedVisits, loadTanks, loadOutlets,
